...
exports.risorse = function(config, urlArray) {
	...
	if (typeof (service) !== 'undefined') {
		er = false;
		body = "<?xml version=\"1.0\" encoding=\"UTF-8\" ?><TileMapService version=\"1.0.0\" services=\"http://"
		...
		for (index in service.tilemaps) {
			...
			body += "<TileMap title=\"" + tilemaps.title + "\" srs=\"" + tilemaps.SRS
					+ "\" profile=\"" + tilemaps.profile;
			body += "\" href=\"http://" + config.host + ":" + config.port + "/"
					+ urlArray[0] + "/" + index + "\" />"
		}
...
exports.tilemap = function(config, urlArray) {
	...
	if (typeof (service) !== 'undefined') {
		...
		if (typeof (tilemaps) !== 'undefined') {
			...
			body += "<TileMap version=\"1.0.0\" tilemapservice=\"http://"
					+ config.host + ":" + config.port + "/" + urlArray[0] + "\">";
			body += "<Title>" + tilemaps.title + "</Title>";
			body += "<Abstract>" + service.abs + "</Abstract>";
			body += "<SRS>" + tilemaps.SRS + "</SRS>";
			body += "<BoundingBox minx=\"" + tilemaps.boundingbox[0] + "\" miny=\""
					+ tilemaps.boundingbox[1];
			body += "\" maxx=\"" + tilemaps.boundingbox[2] + "\" maxy=\""
					+ tilemaps.boundingbox[3] + "\" />";
			body += "<Origin x=\"" + tilemaps.origin[0] + "\" y=\""
					+ tilemaps.origin[1] + "\" />";
			body += "<TileFormat width=\"" + tilemaps.TileFormat[0] + "\" height=\""
					+ tilemaps.TileFormat[1];
			body += "\" mime-type=\"" + tilemaps.TileFormat[2] + "\" extension=\""
					+ tilemaps.TileFormat[3] + "\" />";
			body += "<TileSets profile=\"" + tilemaps.profile + "\">";
			for (upp in tilemaps.tilesets) {
				body += "<TileSet href=\"http://" + config.host + ":" + config.port
						+ "/" + urlArray[0] + "/" + urlArray[1] + "/" + upp
				body += "\" units-per-pixel=\"" + tilemaps.tilesets[upp]
						+ "\" order=\"" + upp + "\" />"
			}
...
// The client requests a nonexistent resource URL. Return HTTP error code 404
// (Not Found)
// The server fails in processing a response for a valid resource URL. Return
