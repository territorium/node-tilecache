 ...
 * @param json
 */
rs.toXML = function(tag, json) {
	var content = '';
	if (typeof json === 'object' && json.length != undefined) {
		for ( var i = 0; i < json.length; i++) {
			content += rs.toXML(tag, json[i]);
		}
		return content;
	} else if (typeof json === 'object') {
		for ( var key in json) {
			content += rs.toXML(key, json[key]);
		}
	} else {
		content = '' + json;
	}
...
rs.onRequest = function(caller, context, request, response) {
	...
	var requestURI = request.url.split('?');
	if (routes) {
		for ( var regexp in routes) {
			var route = routes[regexp];
			var match = requestURI[0].match(new RegExp(regexp));
			if (match && match[0] == requestURI[0]) {
	...
	request.getURL = function(path) {
		var url = 'http://' + request.headers.host + requestURI[0];
		url += (url.charAt(url.length - 1) == '/') ? '' : '/';
		url += (path.length > 0 && path.charAt(0) == '/') ? path.substr(1)
				: path;
		return url;
	};
	...
	request.marshall = function(res, json) {
		if (this.headers.accept.indexOf('application/json') < 0) {
			res.setHeader('Content-Type', 'application/xml');
			res.end(rs.toXML('response', json));
		...
		try {
			...
			if (request.method === 'POST' || request.method === 'PUT') {
				...
				request.on('end', function() {
					handler.apply(caller, params.concat(content));
				});
			} else {
