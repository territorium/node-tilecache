...
function addHandler(context, method, path, callback) {
	...
	var route = {
		"path" : resource,
		"params" : parseParameter(resource),
		"callback" : callback
	};
...
function onRequest(caller, context, request, response) {
	var callback = getCallback(context, request);
	if (callback) {
		try {
			response.statusCode = 200;
			callback.apply(caller, [ request, response ]);
		} catch (e) {
...
/**
 * Parse the path parameters from resource path
 * @param resource
 * @returns
 */
function parseParameter(resource) {
	var params = resource.match(/:(\w*)/gi);
	if (params) {
...
/**
 * Lookup for the callback
 * @param context
 * @param request
 */
function getCallback(context, request) {
	request.params = {};
