...
tms.getTileBounds = function(grid, z, y, x) {
	...
	var width = grid.size[0] * resolution;
	var height = grid.size[1] * resolution;
	var x = grid.origin[0] + (x * width);
	var y = grid.origin[1] + (y * height);
 ...
 * 
 * @param level - the caching level
 * @param x - the horizontal matrix index
 * @param y - the vertical matrix index
	...
	for ( var cache in service.caches) {
		...
		for ( var i = 0; i < tilemap.grids.length; i++) {
			...
			res.write('<TileMap title="' + tilemap.title + '" profile="'
					+ tms.getProfile(conf, gridname) + '" href="' + req.getURL(cache)
					+ '/' + gridname + '" />');
		}
	}
	res.write('</TileMaps>');
 ...
 * @param ext
 */
tms.getTile = function(conf, req, res, name, cache, gridname, z, y, x, ext) {
	...
	map.renderAsync(bbox, grid.size, ext, function(err, data) {
...
exports.bind = function(resource, conf) {
	...
	resource.onGet('/:cache/:grid/:z/:y/:x.:ext', function(req, res, name, cache,
			grid, z, y, x, ext) {
		tms.getTile(conf, req, res, name, cache, grid, z, y, x, ext);
	});
};