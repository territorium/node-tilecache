/**
 * Imported from https://github.com/bpedro/node-fs/
 * 
 * Right now, the following new operations are supported:
 * 
 * <pre>
 * if the 'recursive' parameter is true, creates a directory recursively;
 * mkdir(path, mode, [recursive], [callback])
 * 
 * if the 'recursive' parameter is true, synchronously creates a directory recursively.
...
(function() {
	...
	var fs = require('fs'), mkdirOrig = fs.mkdir, mkdirSyncOrig = fs.mkdirSync, osSep = process.platform === 'win32' ? '\\'
			: '/';

	/**
	 * Offers functionality similar to mkdir -p
	 * 
	 * Asynchronous operation. No arguments other than a possible exception are
	 ...
	 * 
	 * If the third parameter is boolean and true assume that caller wants
	 * recursive operation.
	 */
