...
renderer.Map = function(source, proj, margin) {
	...
	/**
	 * Renders an image and store it on file system and/or return the data to the
	 * callback
	 * @param bbox
	 * @param size
	 * @param format
	 ...
	 * @param callback
	 */
	this.render = function(bbox, size, format, filename, callback) {
		this.doRender(bbox, size, function(err, image) {
			...
			if (filename) {
				...
				fs.writeFileSync(filename, view.encodeSync(format));
				if (callback) callback(err, fs.readFileSync(filename));
			} else if (callback) {
