 ...
 * @param json
 */
seed.createTask = function(service, cache, grid, json) {
	return task = {
...
seed.addTask = function(conf, req, res, content) {
	...
	if (json.service && json.cache && json.grid) {
		...
		var task = seed.createTask(service, cache, grid, json);
		seed.task[task.id] = task;
