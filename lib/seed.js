 ...
 * @param res
 */
seed.getCompleted = function(conf, req, res) {
	var completed = [];
	for ( var id in seed.completed) {
		completed.push(seed.completed[id].options);
	}
	req.marshall(res, {
		completed : completed
	});
...
//			console.log('>>>', m);
			seed.task[task.id].options.created = m.created;
			seed.task[task.id].options.ignored = m.ignored;
			seed.task[task.id].options.error = m.error;	
if (seed.task[task.id].options.created + seed.task[task.id].options.ignored + seed.task[task.id].options.error == seed.task[task.id].options.total){
seed.completed[task.id] = seed.task[task.id];
delete seed.task[task.id];
}		
		});
		child.options = task;
...
exports.bind = function(resource, conf) {
	...
	resource.onGet('/completed', function(req, res) {
		seed.getCompleted(conf, req, res);
	});
