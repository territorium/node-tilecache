exports.mappa = function(config, urlArray) {
	...
	if (typeof (service) !== 'undefined') {
		...
		if (typeof (tilemaps) !== 'undefined') {
			if ((urlArray[2] >= 0)
					&& (urlArray[2] < (tilemaps.tilesets.length))) {
				er = false;
				...
				var numlevely = Math
						.ceil((tilemaps.boundingbox[3] - tilemaps.boundingbox[1])
								/ (ydim * res));
				if (urlArray[3] > (numlevelx - 1)
						|| urlArray[4] > (numlevely - 1)) {
					var body = 'Index out of bounds: Max x level = '
							+ (numlevelx - 1) + ' - Max y level = '
							+ (numlevely - 1);
					er = true;
				...
				var percorso = cache.coordConvert(config.cache_dir,
						...
						numlevely, tilemaps.TileFormat[3]);
				...
				fs.readFile(root, function(err, buffer) {
					...
					if (err) {
						...
						// mkdir.makedir(config.baseurl + percorso.slice(0,
						// -1).join('/'), function(err){
						// if (err) {er = true;
						...
						map.load(stylesheet, function(err, map) {
							if (err) {
								...
								// option);
								map.renderFile(path.resolve(root), option,
										function(err, buffer) {
											if (err) {
												console.log(err);
											}
