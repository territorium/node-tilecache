		...
		.createServer(function(request, response) {
			...
			if (url == "/") {
				...
				if (url_prova.pathname !== url_prova.path) {
					...
					response.setHeader("Content-Type", "text/xml");
					// not_found = wxml.tilemap(response, body, config,
					// urlArray);
									...
									if (err) {
										...
										response
												.writeHead(
														200,
														{
															'Content-Type' : config.services[urlArray[0]].tilemaps[urlArray[1]].TileFormat[2]
														});
										response.end(buffer);
									}
								});
				...
				if (urlArray[2] == 'create') {
					...
					var yfin = urlArray[7];
					var i = 0, j = 0;
					// var newArray = [urlArray[0], urlArray[1], urlArray[3],
					// '0', '1'];
					// not_found = mappe.mappa(response, config, newArray);
					for (i = xini; i <= xfin; i++) {
						for (j = yini; j <= yfin; j++) {
							var newArray = [ urlArray[0], urlArray[1],
									urlArray[3], i + '', j + '' ];
			...
			seed.on('message', function(m) {
				count++;
				console.log('Elaborato il file :' + count + ' di ' + m.tot
						+ 'alle ' + m.ora);
			});
		});
