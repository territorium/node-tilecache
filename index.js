...
var server = http.createServer(function(request, response) {
	...
	var not_found = true; // nel caso in cui la url richiesta non
	// corrisponda a nulla
	...
	if (url.pathname == "/") {
		...
		if (url.pathname == url.path) {
			...
			} else if (url.query.op == 'int') seedInt(url);
		}
		...
		mappe.mappa(config, urlArray, false, function(err, buffer) {
			if (err)
				...
				writeRes(200, 'image', buffer,
						config.services[urlArray[0]].tilemaps[urlArray[1]].TileFormat[2]);
		});
	}
	// else if (not_found) {
			...
			if (url.search == '?op=int') {
				...
				for (index in seedProc) {
					...
					seedProc[index].on('message', function(m) {
						...
						body += 'To level: ' + m.to + '\n';
						body += 'Info: File totali: ' + m.tot + ' - file in elaborazione: '
								+ m.incorso + ' - file in coda: ' + m.incoda
								+ ' - file elaborati:' + ((m.tot - m.incorso) - m.incoda)
								+ '\n';
						console.log(body);
